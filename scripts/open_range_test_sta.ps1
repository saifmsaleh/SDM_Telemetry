...
function Start-RawSerialMonitor {
    ...
    try {
        $serial.Open()
        Write-Host "Range-test STA raw serial monitor on $TargetPort @ $TargetBaud. Press Ctrl+C to stop." -ForegroundColor Yellow
        while ($true) {
            try {
...
$elfPath = Join-Path $repoRoot "examples\range_test_sta\build\range_test_sta.elf"
...
if (-not (Test-Path -LiteralPath $elfPath)) {
    Write-Warning "Range-test STA ELF not found, falling back to raw serial monitor: $elfPath"
    Start-RawSerialMonitor -TargetPort $Port -TargetBaud $Baud
    exit 0
}
